 ...
 * @constructor
 */
class AsyncTreeGridDataSource {
    constructor(treesource, options) {
    ...
    findShadowNodeForId(id, searchIn) {
     ...
     * @returns number
     */
    getShadowNodeSubTreeSize(shadowNode) {
        var self = this;
    ...
    _expandShadowNode(shadowNode, row) {
    ...
    _collapseShadowNode(shadowNode) {
        if (this.view === undefined) {
            throw "Collapse node but view isn't loaded";
        }
        if (!shadowNode.expanded) {
...
export default AsyncTreeGridDataSource;
